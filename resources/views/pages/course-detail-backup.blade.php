...
document.addEventListener('DOMContentLoaded', function() {
  async function initializeCourse() {
    ...
    try {
      const response = await fetch(`{{ url('/api/course-content') }}/${slug}`);
      if (!response.ok) {
        ...
        questions.forEach((q, index) => {
            questionsHtml += `
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h4 class="font-medium text-gray-900 mb-3">Q${index + 1}. ${q.q}</h4>
                    <div class="space-y-2">
            `;
            ...
            q.options.forEach((option, optIndex) => {
                questionsHtml += `
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="radio" name="q${index}" value="${optIndex}" class="text-emerald-600 focus:ring-emerald-500">
